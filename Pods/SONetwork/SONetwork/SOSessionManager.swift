        ...
        let dataTask = self.session.dataTask(with: request as URLRequest) {data,response,error in
            if (error != nil) {
                ...
                if (response as! HTTPURLResponse).statusCode  == 200{
                    ...
                    if let serverResponse = try? JSONDecoder().decode(KHError.self, from: data!){
                        
                        let errorR = NSError(domain: "", code: 0, userInfo: [NSLocalizedDescriptionKey : serverResponse.status?.otherTxt ?? "Somthing Wrong"])
                        fail(errorR as NSError?)
...
struct KHError: Codable {

  enum CodingKeys: String, CodingKey {
    case result = "otherTxt"
      case status
  ...
  init(from decoder: Decoder) throws {
    ...
    if let value = try? container.decode(Int.self, forKey:.result) {
result = String(value)
}else if let value = try? container.decode(String.self, forKey:.result) {
 result = value
}
  }
