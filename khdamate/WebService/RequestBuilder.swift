...
public class RequestOperationBuilder{
    ...
    func headers(_ heads: [String:String]?) -> RequestOperationBuilder {
        self.headers = heads
        return self
    }
    ...
    func alamofireResponseData(_ response:DataResponse<Any>){
        ...
        case .success(let data):
            if data is NSDictionary {
                ...
                if (responseJson.status?.success == true) {
        ...
        debugPrint("[RequestsHeadrs] : \(response.request?.allHTTPHeaderFields?.description ?? "") \n",response)
    }
    #endif
    public func build() -> RequestOperationBuilder {
        #if canImport(Alamofire)
        var heads =  BaseModel.sharedInstance.requestsHeadrs()
        ...
        if self.multiPartFiles.count > 0 {
            ...
            operation = Alamofire.request(GlobalConstants.APIUrl + url!, method: method, parameters: (params as! Parameters), headers:heads).validate(contentType: ["application/json"])
            ...
            operation?.setWillMapDeserializedResponseBlock({ (json) -> Any? in
                ...
                if json is NSDictionary {
                    for keySTR in (json as! NSDictionary).allKeys{
                        for (keyResponseSTR,value) in (self.responseReplacing! as NSDictionary){
                            if (keySTR as! String) == (keyResponseSTR as! String) {
                                jsonNew[(value as! String)] = (json as! NSDictionary)[(keySTR as! String)]
                            }else{
                ...
                if let value = self.beforeSave?(jsonNew.count > 0 ? jsonNew : json) {
                    if value is NSDictionary {
                        jsonNew = (value as! NSDictionary).mutableCopy() as! NSMutableDictionary
                    }else if value is NSMutableDictionary {
                        jsonNew = value as! NSMutableDictionary
                    }
                }
                return jsonNew
            })
