...
open class SO: NSObject {
    
    open class func request( _ method : String = "GET", url : String , header:HTTPHeaders, parameters : Dictionary<String,AnyObject> = [:], success : @escaping (_ result : String?)->Void, fail:@escaping (_ error : NSError?)->Void){
        
        SOSessionManager.default.request(method, url: url, parameters: parameters,headers: header, success: { (data) in
            //此处只返回最原始的数据
            let res = String(data: data!, encoding: String.Encoding.utf8)
            success(res!)
        }) { (error) in
